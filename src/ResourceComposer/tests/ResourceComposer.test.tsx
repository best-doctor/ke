...
test('ResourceComposer mounts router with children', () => {
  const wrapper = shallow(
    <ResourceComposer withSideBar={false} permissions={[]}>
      ...
      <AdminResource name="test" admin={testAdmin} provider={testProvider} user={undefined} />
      <AdminResource name="test2" admin={testAdmin} provider={testProvider} user={undefined} />
    </ResourceComposer>
  )
...
test('Resource mounts properly', () => {
  const wrapper = shallow(<AdminResource name="test" admin={testAdmin} provider={testProvider} user={undefined} />)
